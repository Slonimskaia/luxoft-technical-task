...
public interface ItemRepository extends CrudRepository<Item, Integer> {
    ...
    @Query(value = "SELECT * FROM ITEMS INNER JOIN CSV_FILES ON CSV_FILES.ID = ITEMS.FILE_ID " +
            "WHERE CSV_FILES.FILE_NAME = :fileName", nativeQuery = true)
    List<Item> getFileItems(@Param("fileName") String fileName);

    @Query(value = "SELECT * FROM ITEMS INNER JOIN CSV_FILES ON CSV_FILES.ID = ITEMS.FILE_ID " +
            "WHERE ITEMS.PRIMARY_KEY = :primaryKey AND CSV_FILES.FILE_NAME = :fileName", nativeQuery = true)
    ...
    @Query(value = "DELETE FROM ITEMS WHERE ITEMS.PRIMARY_KEY = :primaryKey " +
            "AND FILE_ID IN (SELECT ID FROM CSV_FILES WHERE FILE_NAME = :fileName)", nativeQuery = true)
